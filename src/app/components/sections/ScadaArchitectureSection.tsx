...
const ScadaArchitectureSection: React.FC<ScadaArchitectureSectionProps> = () => {
  return (
    <section id="scada-architecture" className="px-4 py-8 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <div className="mx-auto w-full max-w-7xl">
        <div>
          <p className="[font-family:var(--font-ibm-plex-sans)] text-[0.875rem] font-medium uppercase leading-[1] tracking-[0.02rem] text-[#FF7F00] sm:text-[0.9375rem]">
            ARCHITECTURE
          </p>
          <h2 className="mt-3 max-w-[720px] [font-family:var(--font-ibm-plex-sans)] text-[1.5rem] font-bold leading-[1.2] tracking-[-0.03rem] text-[#1C1B1B] sm:text-[1.9rem] lg:text-[2.35rem] lg:leading-[1.16] lg:tracking-[-0.05rem]">
            The Digital Nervous System Model
          </h2>
          <p className="mt-3 max-w-[760px] [font-family:var(--font-ibm-plex-sans)] text-[15px] leading-[1.4] text-[#7F7F7F] sm:text-base">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 sm:mt-6 sm:grid-cols-2 lg:mt-8 lg:grid-cols-3 lg:gap-8">
          {layers.map((layer) => (
            <article key={layer.label} className="flex flex-col items-start">
              <div className="flex h-[140px] w-full items-center justify-start sm:h-[150px]">
                <Image
                  src={layer.image}
              ...
              <p className="mt-3 [font-family:var(--font-ibm-plex-sans)] text-[0.75rem] font-medium uppercase leading-[1] tracking-[0.02rem] text-[#FF7F00] sm:text-[0.8125rem]">
                {layer.label}
              </p>
              <h3 className="mt-1.5 [font-family:var(--font-ibm-plex-sans)] text-[1.125rem] font-bold leading-[1.15] tracking-[-0.02rem] text-[#1C1B1B] sm:text-[1.25rem]">
                {layer.title}
              </h3>
              <p className="mt-2 [font-family:var(--font-ibm-plex-sans)] text-[13px] leading-[1.4] text-[#7F7F7F] sm:text-[14px]">
                {layer.description}
              </p>
              ...
              <ul className="mt-3 space-y-1">
                {layer.bullets.map((bullet) => (
                  <li
                    key={bullet}
                    className="flex items-start gap-2 [font-family:var(--font-ibm-plex-sans)] text-[13px] leading-[1.4] text-[#4E4E4E] sm:text-[14px]"
                  >
