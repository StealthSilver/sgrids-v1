...
const EmsGlanceSection: React.FC<EmsGlanceSectionProps> = () => {
  return (
    <section id="ems-at-a-glance" className="px-4 py-8 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <div className="mx-auto w-full max-w-7xl">
        ...
        <p className="mt-3 max-w-[720px] [font-family:var(--font-ibm-plex-sans)] text-[15px] leading-[1.4] text-[#7F7F7F] sm:text-base">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {glanceItems.map((item) => (
            <article
              key={item.title}
              className="flex flex-col rounded-[14px] border border-[#EFE6DF] bg-[#FFFFFF] px-5 py-4 shadow-[0px_7.72px_9.64px_0px_#FF6A0026] transition-all duration-200 hover:-translate-y-1 hover:border-[#FF7F0052] hover:shadow-[0px_12px_18px_0px_#FF6A0038] sm:px-5 sm:py-5"
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-[0.75rem] bg-[#FF7F0026] text-[#FF7F00]">
                {item.icon}
              </div>
              <h3 className="mt-3 [font-family:var(--font-ibm-plex-sans)] text-[18px] font-semibold leading-[1.2] text-[#131313]">
                {item.title}
              </h3>
              <p className="mt-1.5 [font-family:var(--font-ibm-plex-sans)] text-[15px] leading-[1.4] text-[#4E4E4E]">
                {item.description}
              </p>
            </article>
          ))}
